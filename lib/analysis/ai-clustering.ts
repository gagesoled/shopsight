...
async function clusterEmbeddings(embeddings: EmbeddingResult[]): Promise<HierarchicalCluster[]> {
  ...
  if (noiseCluster.length > minPts) {
    hierarchicalClusters.push({ terms: noiseCluster, level: 0, similarity: 1 })
  }
...
Generate a response in JSON format with the following structure:
{
  ...
  if (!messageContent) {
    throw new Error("Failed to generate cluster description")
  }

  try {
    const parsedContent = JSON.parse(messageContent)
    return parsedContent as ClusterDescription
  } catch (error) {
    console.error("Failed to parse cluster description:", error)
    throw new Error("Failed to parse cluster description")
  }
}
...
async function extractSemanticTags(
  ...
  try {
    const responseContent = response.choices[0]?.message?.content || '{"tags":[]}'
    const result = JSON.parse(responseContent)
    return result.tags || []
  } catch (error) {
